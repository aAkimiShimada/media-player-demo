...
import {
	enableMSE, enableEME, enableAutoResolution,
	resolutions,
} from "./consts";
import { videoSizeMessenger } from "./misc";
import {
	resolutionSelectionMessenger,
	playerStateMessenger
} from "./mse";
import { type ResolutionSelection } from "./mse_utils";
import { div, elem, svgElem } from "./interface_utils";
import {
	createMessenger, type Messenger
} from "./utils";
...
const appearanceMessenger = createMessenger<Appearance>("auto");
...
const layoutMessenger = createMessenger<Layout|null>("vertical");
...
const setBodyClass = (body: HTMLBodyElement) => {
	// アピアランスの変更を監視
...
/** コントローラ全体を作成する */
const createController = (): HTMLDivElement => {
	return div({
		id: "controller",
		children: [
			...createCSSVarsDefs(),
			createControllerItems(),
			createControllerFooter()
		]
	});
};

/** CSS 変数を定義するスタイルを作成する */
const createCSSVarsDefs = (): HTMLStyleElement[] => {
	// 2つの `style` 要素を生成する
...
const createControllerItems = (): HTMLDivElement => {
	return div({
		id: "controller-items",
		children: [
			createTagsItem(),
			createResolutionSelectorItem(),
			...createPlayerStateItems()
		].filter(item => item != null)
	})
};

/** 各種設定の有効/無効をタグの有無として表現するリスト項目を用意する */
const createTagsItem = (): HTMLDivElement => {
	const tags: HTMLDivElement[] = [];

	if (enableMSE) {
		tags.push(div({ class: ["tag", "mse"] }));
	}
	if (enableMSE && enableAutoResolution) {
		tags.push(div({ class: ["tag", "auto-res"] }));
	}
	if (enableEME) {
		tags.push(div({ class: ["tag", "eme"] }));
	}
...
const createResolutionSelectorItem = (): HTMLDivElement | null => {
	...
	const getValue = (): ResolutionSelection => {
		// `string` 型の生の値
		...
		// 手動の場合
		if (
			(resolutions as unknown as string[])
			.includes(rawValue)
		) return rawValue as ResolutionSelection;
		// 不適切な値の場合はデフォルト値をセットする
		const value: ResolutionSelection =
...
const createPlayerStateItems = (): HTMLDivElement[] => {
	// MSE が無効な場合はこのメニューは表示しない
	if (!enableMSE) return [];
	...
	/** リストの項目を作成する */
	const createItem = (key: string, value: HTMLDivElement) => div({
		class: ["item"],
		children: [
			div({
				class: ["key"],
	...
	// item を生成して返す
	return [
		createItem("再生中の解像度", videoSize),
		createItem("再生位置", currentTime),
		createItem("最後に読み込んだセグメント", lastLoadedSegmentIndex),
		createItem("次に読み込むセグメント", nextLoadingSegmentIndex),
		createItem("セグメントの解像度", resolution),
		createItem("オフライン状態", isOffline)
	];
};
...
/** ボタンのアイコンを構成する DOM */
const icons: { [key in Appearance|Layout]: SVGSVGElement } = (() => {
	/**
	 * アピアランス表示用の SVG アイコンを作成する
	 *
	 * 内部の `path` 要素の `d` 属性だけ引数として与えれば生成できる
	 */
	const makeAppearanceIcon = (d: string) => (
		svgElem("svg", {
...
/** コントローラのフッター部分を作成する */
const createControllerFooter = (): HTMLDivElement => {
	return div({
		id: "controller-footer",
		children: [
			// アピアランス
			createButtonGroup([
				["auto", "システムの設定に合わせて自動的にライト/ダークを選択します。"],
				["light", "ライトモードで表示します。"],
				["dark", "ダークモードで表示します。"],
			], appearanceMessenger),
			// レイアウト
			createButtonGroup([
				["vertical", "コントローラを動画の下側に表示するようにします。"],
				["horizontal", "コントローラを動画の右側に表示するようにします。"]
			], layoutMessenger)
		]
	});
};
 ...
 * */
const createButtonGroup = <T extends keyof (typeof icons)>(
	info: [value: T, description: string][],
