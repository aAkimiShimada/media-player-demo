...
import {
	...
	base64ToHex, binaryToHex, hexToBase64,
	type KeyPair
} from "./eme_utils";
import {
	createMutexManager,
...
const configs: MediaKeySystemConfiguration[] = [
	{
		// 初期化データの種類
		initDataTypes: ["cenc"], // Common Encryption Scheme
...
/**
 * Clear Key を使用する場合に CDM からライセンス提供を要求される時のデータ形式の型。この形式の JSON が CDM から渡される。
 *
 * 仕様: https://w3c.github.io/encrypted-media/#clear-key-request-format
 */
interface ClearKeyLicenseRequestFormat {
	/** key id のリスト。 Base64 形式になっている。 */
	kids: string[];
	/** セッションの形式 */
...
/**
 * Clear Key を使用する場合に CDM にキーを渡す場合に使用するデータ形式の型。 JSON として渡す。 JWK (JSON Web Key) と呼ばれる形式である。
 *
 * 仕様: https://w3c.github.io/encrypted-media/#clear-key-license-format
 */
interface ClearKeyLicenseFormat {
	/** キーのリスト */
	keys: {
		/** キー ID (key id) */
		kid: string;
 ...
 */
export const setupEME = (video: HTMLVideoElement) => {
 ...
 * * 第1引数: `video` 要素の DOM (`HTMLVideoElement`)
 */
const getMediaKeys = async (video: HTMLVideoElement): Promise<MediaKeys> => {
	...
	const mediaKeys =
		await keySystemAccess.createMediaKeys()
		.catch(error => {
 ...
 */
const passEncryptionKeys = (session: MediaKeySession, event: MediaKeyMessageEvent) => {
	...
	LOG([
		"CDM からライセンスリクエストのメッセージを受け取りました",
		`メッセージの内容: ${messageJson}`
	].join("\n"), "EME");

	// メッセージは JSON であるから、パースして所定の形式のオブジェクトとして取り扱う
	const message = JSON.parse(messageJson) as ClearKeyLicenseRequestFormat;

	// 用意した `{ video: KeyPair, audio: KeyPair }` の形式のオブジェクトから、 `KeyPair` だけを取り出して `KeyPair[]` の形式にしておく
	const keyPairs = Object.values(encryptionKeys) as KeyPair[];

	/** キーリストの型 */
	type KeysList = ClearKeyLicenseFormat["keys"];
	...
	// CDM に返答する内容を用意する
	const update: ClearKeyLicenseFormat = {
		keys: keysList,
 ...
 * * 第1引数: CDM とやり取りするセッション
 */
const changeKeyStatusOfSession = (session: MediaKeySession) => {
	// キーごとのステータスがまとまった辞書
	...
	.map(([bsKid, status]) => {
		...
		const binKid = arrayBufferToString(bsKid as ArrayBuffer);
