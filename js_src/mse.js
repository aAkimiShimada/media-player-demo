...
import { audioMimeType, maxBufferDuration, segmentCount, segmentDuration, videoMimeType, resolutionIndexes, advanceSegmentsAfterSeekEnd, advanceSegmentsAfterWaiting, resolutions, getVideoSegmentUrl, getAudioSegmentUrl, getOptimalResolution, enableAutoResolution } from "./consts.js";
import { fetchAsArrayBuffer, createMutexSourceBuffer, createTimer } from "./mse_utils.js";
import { createTaskManager, createMessenger } from "./utils.js";
import { LOG, ERROR } from "./log.js";
/** ユーザーが選択した解像度の設定を管理するメッセンジャー */
export const resolutionSelectionMessenger = createMessenger(enableAutoResolution ? "auto" : resolutions[0]);

/** 再生状態に関するデータをまとめた `PlayerState` 型を管理するメッセンジャー */
export const playerStateMessenger = createMessenger({
	currentTime: 0.0,
 ...
 */
const startStreaming = (video, mediaSource) => {
	/** 映像と音声のバッファーをまとめたもの: `[videoBuffer, audioBuffer]` */
	const buffers = [videoMimeType, audioMimeType].map(mimeType => {
	...
	const removePreviousSegment = () => {
		...
		const notInBeginning = !videoBuffer.updating() && !audioBuffer.updating() && currentSegment > 1;
		if (notInBeginning) {
	...
	const tryFetching = () => {
		...
		connectionTrialTimer.start(async () => {
			...
			// 今回取得を試みるセグメントのインデクス
			playerState.nextLoadingSegmentIndex = Math.floor(playerState.currentTime / segmentDuration);
			// 変更を通知
			playerStateMessenger.notify();
 ...
 */
const supplyBuffers = async (buffers, taskManager) => {
	// 現在の再生状態を取得する
	const playerState = playerStateMessenger.get();
 ...
 */
const appendBuffers = async (videoResolution, segmentIndex, buffers, taskManager) => {
	// 現在の再生状態を取得する
	const playerState = playerStateMessenger.get();

	// 映像と音声それぞれのリソースを呼び出す URL を取得する
	const videoSegmentUrl = getVideoSegmentUrl(resolutionIndexes[videoResolution], segmentIndex);
	const audioSegmentUrl = getAudioSegmentUrl(segmentIndex);

	// 映像と音声の両方のセグメントの取得を行う
	const [videoSuccess, audioSuccess] = await Promise.all([appendSegmentToBuffer(videoSegmentUrl, buffers[0], taskManager), appendSegmentToBuffer(audioSegmentUrl, buffers[1], taskManager)]);
 ...
 */
const appendSegmentToBuffer = async (segmentUrl, mutexSourceBuffer, taskManager) => {
	try {
